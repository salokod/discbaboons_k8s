...
4. **Sequential Processing**: Holes are processed in starting_hole order for proper carry-over logic
5. **Non-Hole-1 Starting Support**: Rounds starting on any hole (e.g., hole 5) process correctly
...
### Money Tracking (Net Gain/Loss)
The `netGain` field tracks each player's profit or loss from the skins game:

**Calculation**: `netGain = totalValue - totalPotContribution`
- `totalValue`: Total money won from skins victories
- `totalPotContribution`: Total money paid into the pot (holes played × skins value per hole)

**Examples**:
- Player wins $15 in a 4-hole game with $5/hole skins → `netGain = $15 - $20 = -$5.00` (net loss)
- Player wins $25 in a 4-hole game with $5/hole skins → `netGain = $25 - $20 = +$5.00` (net profit)
- Player wins $0 in a 4-hole game with $5/hole skins → `netGain = $0 - $20 = -$20.00` (paid but won nothing)
